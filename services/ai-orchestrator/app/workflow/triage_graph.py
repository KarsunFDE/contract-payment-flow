"""
triage_graph.py — Person B: the multi-agent triage flow (m3.md Phase 7,
Step 7.5 — task B5).

anomaly_detector -> adjudicator -> decision_router, then Command-based
three-lane routing (auto_process / hitl_escalate / return_route). The inner
SF-30 workflow (Phases 0-6) is the HITL processing subgraph.

decision_router returns Command(goto=...) — the lane edges are dynamic, so
there are NO static edges out of the router; LangGraph derives the possible
destinations from the node's Command[Literal[...]] return annotation.
...
def build_triage_graph() -> StateGraph:
    ...
    # decision_router routes via Command(goto=...) — no static edges out of it.
