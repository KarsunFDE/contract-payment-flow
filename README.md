...
**Task portability**: daily-tasks given against `contract-payment-flow` apply to **all 4 repos** because the 12 brownfield-debt items + architecture + file-pattern shape are constant. Each pair-project's `domain-mapping.md` (auto-generated by `pair-brownfield-generator` Step 5) is the rename-lookup so a task spec like "Fix audit-log race in `contract-modification-service`" auto-translates to each pair's renamed service. See [`training-resources/templates/eod-deliverable.md`](https://github.com/KarsunFDE/training-resources/blob/main/templates/eod-deliverable.md) for the portability-matrix authoring shape.
...
```bash
# 1. Clone
git clone https://github.com/KarsunFDE/contract-payment-flow.git
cd contract-payment-flow
...
curl http://localhost:8081/actuator/health        # contract-modification-service
curl http://localhost:8082/actuator/health        # invoice-review-service
...
2. Audit-log written *after* response in contract-modification-service (race on crash)
3. No circuit breaker in invoice-review-service → contract-modification-service
...
8. Frontend `ContractModificationListComponent` hardcodes `http://localhost:8081/api/contract-modifications` (bypasses gateway)
9. No OWASP input sanitization on contract_modification `description` (accepts raw HTML)
...
| W1 Thu–Fri | First LLM PRs land in `services/ai-orchestrator/` against the contract_modification-drafting endpoint. |
