...
Contracting Officers (COs) performing post-award contract modifications under FAR Part 43 must complete SF-30 forms. Errors in rationale language, clause citations, and modification justifications create compliance risk and payment delays downstream. The project needs an AI-assisted interface that helps COs draft SF-30 content grounded in the actual FAR Part 42/43/32 corpus — not free-form generation.

Scope is strictly **post-award modifications** (FAR Part 43). Pre-award solicitation amendments (SF-30 "Amendment of Solicitation" use) are **out of scope**.
...
- Corpus: FAR Part 42, 43, and 32 clause text.
- Confidence threshold: **0.85** — drafts below threshold are withheld, never shipped.
- Every AI-drafted rationale must cite a real FAR clause traceable to a retrieved corpus excerpt. The system must never auto-generate a citation it cannot trace to a retrieved source.
...
- Retrieved FAR clause IDs and excerpts
...
### Audit Log
- Append-only. Each entry captures: input payload, model ID + version, retrieved FAR clauses + excerpts, citations used, CO identity + role + timestamp + decision.
- Must not replicate the audit-log race bug documented in `pair-unique-debt.md`.

## Alternatives Considered

1. **Free-form LLM generation (no RAG grounding).** Rejected — ungrounded FAR citations are a compliance violation. CO cannot verify a citation that has no traceable corpus source.
...
- FAR Part 42/43/32 corpus must be embedded and indexed before any Bedrock invocation is attempted.
...
If RAG grounding proves unreliable against the FAR corpus (persistent sub-threshold confidence), the helper degrades gracefully: present the CO with the raw retrieved clause excerpts and an empty draft field rather than a hallucinated fill. CO completes the form manually. Audit log still captures the retrieval attempt, confidence score, and CO decision. No silent failure path.
